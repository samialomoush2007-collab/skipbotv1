...
async def handle_autorep_command(original_msg: str, sender_uid: int, chat_id: int, chat_type: int,
    ...
    if not original_msg.startswith('/samiautorep '):
        return
...
async def handle_love_command(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    if not inPuTMsG.strip().startswith('/samilove2 '):
        return
    parts = inPuTMsG.strip().split(maxsplit=1)
    if len(parts) < 2:
...
async def handle_gop_command(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    if not inPuTMsG.strip().startswith('/samigop '):
        return
...
async def handle_commands(cmd_text, sender_uid, chat_id, chat_type, key, iv, region):
    ...
    if cmd_text.startswith('/samistart1'):
    ...
    elif cmd_text.startswith('/samistop1'):
        stop_auto = True
...
async def handle_tagx_command(inPuTMsG: str, uid: int, chat_id: int, chat_type: int, key: bytes, iv: bytes, region: str):
    ...
    if not inPuTMsG.strip().startswith('/samitagx'):
        return
...
# =================== GLOBAL VARIABLES ===================
ADMIN_UID = "2723489019"  # Your admin UID
ALLOWED_UIDS = [ADMIN_UID, "1525805550", "7170494698", "1919429437", "1919429437", "1953276476"]
online_writer = None
whisper_writer = None
spammer_uid = None
...
async def play_emote_spam(uids, number, key, iv, region):
    ...
    try:
        emote_id = ALL_SAMI.get(int(number))
        if not emote_id:
...
📱 Player will see:
"SKIP-CODEX FF  wants to join your team"
...
async def clan_message_loop():
    """Disabled clan message loop"""
    global _clan_msg_key, _clan_msg_iv, _clan_msg_bot_uid, _clan_msg_clan_id
    try:
        # Wait a short moment for connection to stabilize
        await asyncio.sleep(5)
        
        # تم تعطيل الإرسال هنا
        #print("[CLAN] Message loop is disabled.")
        
        # Loop every 5 hours
        while True:
            await asyncio.sleep(18000)  # 5 hours = 18000 seconds
            # تم تعطيل الإرسال هنا أيضاً
            #print("[CLAN] Message loop is still running but sending is disabled.")
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg =  f"""[B][C][FFFF00] بوت صنع skip
...
"""
    ...
    try:
        ...
        # Completion message
        completion_msg =  f"""[B][C][FFFF00] على هونك
"""
        await safe_send_message(chat_type, completion_msg, uid, chat_id, key, iv)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith('0500') and insquad is not None and joining_team == False:
                    try:
                        ...
                        # خطأ في الـ Case 1 يتم تجاهله بصمت للحفاظ على استقرار اللوب
                        pass
                ...
                # Case 2: Auto-accept for whitelisted users
                    ...
                    try:
                        ...
                        # التحقق المسبق لمنع ظهور خطأ '5'
                        if '5' not in packet_json or 'data' not in packet_json['5']:
                            continue

                        data_node = packet_json['5']['data']
                        uid = data_node['1']['data']
                        invite_uid = data_node['2']['data']['1']['data']
                        squad_owner = data_node['1']['data']
                        code = data_node['8']['data']
  
                        emote_id = 909040008
                        bot_uid = 5335556614
    
                        # 🎯 التحقق من هوية المالك
                        if str(squad_owner) == str(BOT_OWNER_UID):
                            ...
                            try:
                                ...
                                print(f"❌ Bundle error: {bundle_e}")
                        
                        else:
                            # رفض الدعوة إذا كان المستخدم ليس المالك
                            print(f"🚫 Ignored invite from non-owner: {squad_owner}")
                            msg = "Can't accept Your request. Talk to YASINFF"
                            reject_packet = await xSEndMsg(Msg=msg, Tp=2, Tp2=int(squad_owner), id=int(bot_uid), K=key, V=iv)
                            if whisper_writer and reject_packet:
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', reject_packet)

                    except Exception as e:
                        # التعامل مع أي خطأ غير متوقع في المعالجة
                        insquad = None
                        current_squad_id = None
                        joining_team = False
                        print(f"⚠️ Error in auto-accept case 2 logic: {e}")

                # الجزء الأخير: تحديثات الفريق
...
# =================== TITLE DETECTION (COPY TITLE + EMOTE) ===================
                                    ...
                                    if data_json.get("type") == "Title":
                                        ...
                                        if sender_uid != bot_uid:
                                            ...
                                            if int(sender_uid) == owner_uid:
                                                reply_msg = f"[B][C][00FF00]على هونك"
                                            else:
                                                reply_msg = f"[B][C][00FF00]على هونك"
                    ...
                    if response:
                        ...
                        if inPuTMsG.strip() == '/samicopy_off':
                            emote_hijack = False
...
"""
                        ...
                        # Likes Command - /likes
                        if inPuTMsG.strip().startswith('/samilikes '):
...
#𝙂𝙃𝙊𝙎𝙏 𝘾𝙈𝘿
                        ...
                        if inPuTMsG.strip().startswith('/samighost'):
                            parts = inPuTMsG.strip().split()
                            global GHOST_ENABLED
                        ...
                        if inPuTMsG.strip().startswith('/samifreeze'):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samiswitch5'):
...
# ===== SQUAD CREATION DETECTION (FIXED) =====
                        ...
                        if inPuTMsG.strip().startswith('/samicraftland'):
                        ...
                        if inPuTMsG.strip() == '.hud':
                           ...
                           continue
                        if inPuTMsG.strip().startswith('/samibio'):
...
"""
                        ...
                        if inPuTMsG.strip() in ('.leaveclansami', '.samiexitclan'):
                        ...
                        if inPuTMsG.strip() == '/samistop' and friends_list_running:
                        ...
                        if inPuTMsG.strip().startswith('/samilw'):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samirandom'):
                        ...
                        if inPuTMsG.strip().startswith('/samifast'):
                        ...
                        if inPuTMsG.strip().startswith('/samiquick '):
                        ...
                        if inPuTMsG.strip().startswith('/samixjoin '):
                        ...
                        if inPuTMsG.strip().startswith('/samiinv'):
                        ...
                        # Invite Command - /inv (creates 5-player group and sends request)
                        if inPuTMsG.strip().startswith('/samisnd'):
                            print('Processing invite command in any chat type')
                            ...
                            if len(parts) < 2:
                                ...
                                #GET PLAYER LIKE
                        if inPuTMsG.strip().startswith('/samilike'):
                        ...
                        if inPuTMsG.strip().startswith('/samiwlremove'):
                            parts = inPuTMsG.strip().split()
                            ...
                            # Check owner
                            if str(response.Data.uid) != "2723489019":
                        ...
                        if inPuTMsG.strip() == '/samiwlenable':
                        ...
                        if inPuTMsG.strip() == '/samiwldisable':
                        ...
                        if inPuTMsG.strip().startswith('/samiwladd'):
...
"""
                        ...
                        if inPuTMsG.strip() == '/samiviewguests':
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samiguest'):
...
"""
                        ...
                        if inPuTMsG.startswith('/samimimic_on'):
                            success_msg = f"[B][C][FF0000]The Mimic Is Now On\n"
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            emote_hijack = True
                            
                        if inPuTMsG.startswith('/samimimic_off'):
                            success_msg = f"[B][C][FF0000]The Mimic Is Now OFF\n"
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            emote_hijack = False

                        if inPuTMsG.startswith('/samiblock_emote'):
                        ...
                        if inPuTMsG.startswith('/samiunblock_emote'):
                        ...
                        if inPuTMsG.strip().startswith('/samis_m '):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samifriend '):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samiroom_msg'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                        ...
                        if inPuTMsG.strip().startswith('/samikick'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /kick (uid)\nExample: /kick 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/samitester'):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /kick (uid)\nExample: /kick 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/samijoinroom'):
                            ...
                            if len(parts) < 3:
                                ...
                                try:
                                    ...
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.startswith(("/sami100")):
                        ...
                        if inPuTMsG.strip().startswith('/1234567899 '):
                            #     Admin      
                            print('Processing /12345678 command')

                            try:
                                # شرط التحقق من قائمة الآيديهات المسموحة دون تغيير باقي الكود
                                ...
                                if len(parts) < 3:
                                    ...
                                    try:
                                        ...
                                        for _ in range(spam_count):
                                            #      
                                            import random
                                            r_color = "%06x" % random.randint(0x333333, 0xFFFFFF)
                                            
                                            #     
                                            colored_msg = f"[b][c][{r_color}]{user_message}"
                                            await safe_send_message(response.Data.chat_type, colored_msg, uid, chat_id, key, iv)
                                            await asyncio.sleep(0.3)

                                    except ValueError:
                                        error_num = "[b][c][FF0000] ERROR! Please enter a valid number."
                                        await safe_send_message(response.Data.chat_type, error_num, uid, chat_id, key, iv)

                            except Exception as e:
                                error_log = f"[b][c][FF0000] ERROR! Something went wrong:\n{str(e)}"
                        ...
                        if inPuTMsG.strip().startswith('/رقص '):
                            ...
                            if len(parts) < 3:
                                await safe_send_message(response.Data.chat_type, "❌ الاستخدام: /new [الكود] [رقم_الرقصة]", uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                
                                # تحديد الأيدي والرقصة
                                if len(parts) == 3:
                                    target_uid = uid
                                    emote_key = int(parts[2])
                                else:
                                    target_uid = int(parts[2])
                                    emote_key = int(parts[3])
                                
                                # البحث عن الرقصة في القائمة الكبيرة
                                ...
                                try:
                                    ...
                                    await asyncio.sleep(1) # وقت أطول قليلاً لضمان الاستقرار
                                    ...
                                    #await safe_send_message(response.Data.chat_type, f"✅ تم تنفيذ الرقصة رقم {emote_key} بنجاح!", uid, chat_id, key, iv)
                        ...
                        if inPuTMsG and inPuTMsG.strip().startswith('/نص '):
                            print('Processing /نص command with TOP priority')
                            try:
                                if str(uid) != str(BOT_OWNER_UID):
                                    continue
                                ...
                                # استخدام if/elif بدلاً من القاموس لتجنب مشاكل ترميز النصوص الطويلة
                                message = ""
                                if msg_key == "1":
                                    message = "هههههههههه يا سيدي خير خير خير خير بعين الـٓله ههههههههههههه اه اه اه سمعت عن الموضوع اه ههههههههههه والـِله مشكلة هاي يا حج ههههههههههه قالولي اه قالولي هههه ان شاء الُـله ان شاء الُِله ان شاء اُلـله خير خير ماشي تمام تمام طيب طيب طيب اه طيب ماشي هههههههههههههه لعله خير يا سيدي هههههههههههههه هي الدنيا هيك اه والله هههههههههههه منتا عارف وضع الكوكب يعني ههههههههههه ان شاء الـٕله يا سيدي تمام تمام حبيبي حبيبي بوصل ان شاء الله ءه ءه ءه هلا هلا هههه تمام تمام سء سء سلام"
                                elif msg_key == "2":
                                    message = "هههههههههههههههههههههههههههههههههههههه وربي تصير تصير معليك تصير تصير تصير والـِله تصير تصير وربي تصير تصير معليك تصير تصير تصير والـِله تصير تصير وربي تصير تصير معليك تصير تصير والـِله تصير تصير وربي تصير معليك تصير تصير تصير تصير والـِله تصير وربي تصير تصير معليك تصير تصير تصير والـِله تصير تصير وربي تصير تصير معليك تصير تصير تصير والـِله تصير تصير يولد وربي تصير معليك تصير تصير تصير تصير والـِله تصير تصير وربي تصير معليك تصير تصير تصير تصير تصير والـِله تصير"
                                elif msg_key == "3":
                                    message = "حصل خير - مردها تزين يارجال فتره وتعدي يارجال - تصير - صارت معي وانا اخوك - ربك يحلها - فتره وتزين لعلها خيره حصل خير - مردها تزین یارجال - فتره وتعدي يارجال - تصير - صارت معي وانا اخوك - ربك يحلها - فتره وتزين - لعلحصل خیر - مردها تزین یارجال - فتره وتعدي يارجال - تصير - صارت معي وانا اخوك - ربك يحلها - فتره وتزين لعلها خيره حصل خير - مردها تزين يارجال - فتره وتعدي يارجال - تصير - صارت معي وانا اخوك - ربك يحلها - فتره وتزين - لعل حصل خير - مردها تزين يارجال - فتره وتعدي يارجال - تصير - صارت معي وانا اخوك - ربك يحلها - فتره وتزين - لعلها خيره حصل خير - مردها تزين يارجال - فتره وتعدي يارجال - تصير - صارت معي وانا اخوك ربك يحلها فتره وتزين لعل حصل خير - مردها تزین"
                                
                                if message:
                                    await safe_send_message(response.Data.chat_type, message, uid, chat_id, key, iv)

                            except Exception as e:
                                print(f"❌ Error in /نص: {e}")
...
# ==========================================
                        if inPuTMsG and inPuTMsG.strip().startswith('/لعبه1'):
                            print('Processing /game1 command with TOP priority')
                            try:
                                # شرط التحقق من قائمة الآيديهات المسموحة المعرفة بأول السكريبت
                                if str(uid) not in ALLOWED_UIDS:
                                    not_admin_msg = "[b][c][FF0000]مسوي خوي ؟؟ بس لسامي اذلف"
                                    await safe_send_message(response.Data.chat_type, not_admin_msg, uid, chat_id, key, iv)
                                    continue

                                categories = ["اسم", "حيوان", "نبات", "بلاد", "جماد"]
                                arabic_letters = [
                                ...
                                game_msg = (
                                    "  🎮 [FF0000]لُعْبَةُ الأَحْرُفِ [FFFFFF]🎮\n"
                                    "\n"
                                    "\n"
                                    "  ✨ المطلوب: [FF0000]{category} [FFFFFF]بحرف [FF0000]{letter}\n"
                                    "\n"
                                    "\n"
                                    "  ✨ [FF0000]الي يكتب أسرع يفوز! [FFFFFF]🏃‍♂️\n"
                                ).format(category=selected_category, letter=selected_letter)
                                
                                await safe_send_message(response.Data.chat_type, game_msg, uid, chat_id, key, iv)
                                
                            except Exception as e:
                                error_log = f"[b][c][FF0000]❌ خطأ في اللعبة:\n{str(e)}"
                                await safe_send_message(response.Data.chat_type, error_log, uid, chat_id, key, iv)
                            
                            continue

# ==========================================
# 2. لعبة الكلمة المقلوبة
# ==========================================
                        if inPuTMsG and inPuTMsG.strip().startswith('/لعبه2'):
                            print('Processing /game2 (Reversed Word) command')
                            try:
                                # شرط التحقق من قائمة الآيديهات المسموحة المعرفة بأول السكريبت
                                if str(uid) not in ALLOWED_UIDS:
                                    not_admin_msg = "[b][c][FF0000]مسوي خوي ؟؟ بس لسامي اذلف"
                                ...
                                words_pool = [
                                    "مدرسة", "سفينة", "طيارة", "كمبيوتر", "موبايل", "تفاحة", "صندوق", "سيارة", "كتاب", "طاولة",
                                    "قلم", "دفتر", "مكتبة", "استاد", "ملعب", "مستشفى", "طبيب", "مهندس", "معلم", "طالب",
                                    "جامعة", "كرسي", "شباك", "باب", "مفتاح", "تلفزيون", "ساعة", "نظارة", "حقيبة", "حذاء",
                                    "قميص", "ثوب", "سروال", "قبعة", "شمس", "قمر", "نجوم", "كوكب", "فضاء", "سحاب",
                                    "مطر", "ثلج", "بحر", "نهر", "محيط", "بحيرة", "سمكة", "حوت", "قرش", "عصفور",
                                    "صقر", "نسر", "حمامة", "أسد", "نمر", "فهد", "ذئب", "ثعلب", "كلب", "قطة",
                                    "أرنب", "غزال", "زرافة", "فيل", "قرد", "حصان", "حمار", "جمل", "خروف", "بقرة",
                                    "دجاجة", "ديك", "بطة", "أوزة", "تمساح", "ثعبان", "ضفدع", "عقرب", "عنكبوت", "نملة",
                                    "نحلة", "فراشة", "ذبابة", "بعوضة", "شجرة", "ورقة", "زهره", "وردة", "عشب", "غابة",
                                    "حديقة", "مزرعة", "جبل", "تل", "وادي", "صحراء", "جزيرة", "طريق", "شارع", "جسر",
                                    "نفق", "بيت", "منزل", "شقة", "غرفة", "مطبخ", "حمام", "صالة", "جدار", "سقف",
                                    "أرض", "سجاد", "ستارة", "صورة", "مرآة", "دولاب", "سرير", "وسادة", "لحاف", "ملعقة",
                                    "شوكة", "سكين", "صحن", "كوب", "إبريق", "قدر", "فرن", "ثلاجة", "غسالة", "مروحة",
                                    "مكيف", "مكنسة", "شاحن", "بطارية", "سلك", "لمبة", "فانوس", "شمعة", "كبريت", "نار",
                                    "دخان", "رماد", "حجر", "تراب", "رمل", "طين", "ذهب", "فضة", "حديد", "نحاس",
                                    "خشب", "زجاج", "بلاستيك", "ورق", "كرتون", "قماش", "قطن", "صوف", "حرير", "خبز",
                                    "أرز", "لحم", "دجاج", "سمك", "خضار", "فواكه", "تمر", "عسل", "حليب", "جبن",
                                    "بيض", "زيت", "زبدة", "ملح", "سكر", "بهارات", "ماء", "عصير", "شاي", "قهوة",
                                    "نعناع", "زعتر", "زيتون", "تين", "رمان", "عنب", "موز", "برتقال", "ليمون", "مانجو",
                                    "خوخ", "مشمش", "كرز", "بطيخ", "شمام", "فراولة", "توت", "أناناس", "كيوي", "جوافة",
                                    "برقوق", "تين", "زنجبيل", "قرفة", "كمون", "عدس", "فول", "حمص", "فاصوليا", "بازلاء",
                                    "بصل", "ثوم", "بطاطس", "طماطم", "خيار", "جزر", "خس", "ملفوف", "قرنبيط", "باذنجان",
                                    "كوسا", "فلفل", "بقدونس", "كزبرة", "جرجير", "سبانخ", "لفت", "بنجر", "فجل", "قلقاس",
                                    "صيدلية", "عيادة", "مكتب", "شركة", "مصنع", "متجر", "دكان", "سوق", "مجمع", "فندق",
                                    "مطعم", "مقهى", "مخبز", "محطة", "مطار", "ميناء", "قطار", "حافلة", "مترو", "دراجة",
                                    "شاحنة", "رافعة", "جرار", "قارب", "مظلة", "بالون", "صاروخ", "سهم", "قوس", "درع",
                                    "سيف", "خنجر", "رمح", "مسدس", "بندقية", "مدفع", "قنبلة", "خوذة", "سترة", "حزام",
                                    "محفظة", "سوار", "خاتم", "قلادة", "قرط", "تاج", "مجوهرات", "ماس", "ياقوت", "زمرد",
                                    "لؤلؤ", "مرجان", "عقيق", "بلور", "فحم", "بترول", "غاز", "كهرباء", "طاقة", "رياح",
                                    "سد", "خزان", "أنبوب", "حنفية", "محبس", "عداد", "مطرقة", "كماشة", "مفك", "مسكرة"
                                ]

                                import random
                                original_word = random.choice(words_pool)
                                reversed_word = original_word[::-1]

                                game_msg = (
                                    "[b][c][FFFFFF]🧩 [FF0000]تَحَدِّي الكَلِمَةِ المَقْلُوبَةِ [FFFFFF]🧩\n"
                                    "────────────────────\n"
                                    "✨ الكلمة المقلوبة هي: [FF0000]{rev_word}\n"
                                    "[FFFFFF]────────────────────\n"
                                    "⚡ [FF0000]اكتب الكلمة الصحيحة فوراً لتفوز! [FFFFFF]🏃‍♂️"
                                ).format(rev_word=reversed_word)
...
# ==========================================
# 3. لعبة الحاجتين للمحترفين
# ==========================================
                        if inPuTMsG and inPuTMsG.strip().startswith('/لعبه3'):
                            print('Processing /game3 (Real-Life Challenge) command')
                            try:
                                # شرط التحقق من قائمة الآيديهات المسموحة المعرفة بأول السكريبت
                                if str(uid) not in ALLOWED_UIDS:
                                    not_admin_msg = "[b][c][FF0000]مسوي خوي ؟؟ بس لسامي اذلف"
                                ...
                                import random
                                items_pool = [
                                    "اكتب اسم فاكهتين تحتويان على بذور كثيرة؟",
                                    "اكتب اسم حيوانين يملكان فرواً سميكاً؟",
                                    "اكتب اسم أداة مطبخ نستخدمها لفتح المعلبات؟",
                                    "اكتب اسم نوعين من التوابل نضعها في الطعام دائماً؟",
                                    "اكتب اسم شيئين نستخدمهما لغسل الملابس في الغسالة؟",
                                    "اكتب اسم فاكهتين تحتاجان للتقشير قبل الأكل??",
                                    "اكتب اسم حيوانين يملكان أجنحة ولا يطيران؟",
                                    "اكتب اسم قطعتين من الأثاث موجودة في كل غرفة نوم؟",
                                    "اكتب اسم شيئين نراهما في حقيبة المدرسة؟",
                                    "اكتب اسم جهازين إلكترونيين نستخدمهما للتواصل؟",
                                    "اكتب اسم نوعين من المكسرات المحبوبة؟",
                                    "اكتب اسم حيوانين برمائيين يعيشان في الماء والبر؟",
                                    "اكتب اسم شيئين نستخدمهما لتنظيف الأسنان؟",
                                    "اكتب اسم فاكهتين لونهما أخضر؟",
                                    "اكتب اسم أداة نستخدمها للحديقة (للعشب أو الشجر)؟",
                                    "اكتب اسم شيئين يوجدان في محفظة الجيب؟",
                                    "اكتب اسم نوعين من الطعام يطبخان في الفرن؟",
                                    "اكتب اسم حيوانين يملكان قروناً على رؤوسهما؟",
                                    "اكتب اسم شيئين مصنوعين من البلاستيك في المطبخ؟",
                                    "اكتب اسم أداة نستخدمها لقص الورق أو القماش؟",
                                    "اكتب اسم مشروبين طبيعيين يعصران من الفواكه؟",
                                    "اكتب اسم حيوانين ليليين ينشطان في الظلام؟",
                                    "اكتب اسم شيئين يوجدان في الحمام للتنظيف؟",
                                    "اكتب اسم نوعين من الخضار تستخدم في السلطة؟",
                                    "اكتب اسم شيئين نستخدمهما لتغطية الجسم في الشتاء؟",
                                    "اكتب اسم أداة نستخدمها للكتابة غير القلم؟",
                                    "اكتب اسم حيوانين سريعين جداً في الجري؟",
                                    "اكتب اسم فاكهتين صغيرتي الحجم؟",
                                    "اكتب اسم شيئين مصنوعين من الزجاج نستخدمهما للشرب؟",
                                    "اكتب اسم قطعتين في السيارة نراهما من الداخل؟",
                                    "اكتب اسم نوعين من الحلويات التي تعتمد على العجين؟",
                                    "اكتب اسم حيوانين بحريين يملكان زعانف؟",
                                    "اكتب اسم شيئين نستخدمهما لتثبيت الصور أو الأوراق؟",
                                    "اكتب اسم أداة نستخدمها لتنظيف أرضية البيت؟",
                                    "اكتب اسم فاكهتين ذات قشرة قاسية؟",
                                    "اكتب اسم حيوانين يملكان ذيلاً طويلاً؟",
                                    "اكتب اسم شيئين يوجدان في غرفة المعيشة؟",
                                    "اكتب اسم مشروبين يفضل شربهما ساخنين؟",
                                    "اكتب اسم أداة نستخدمها لتمشيط الشعر؟",
                                    "اكتب اسم شيئين نستخدمهما لتصفيف أو ربط الشعر؟",
                                    "اكتب اسم فاكهتين حلوتي المذاق؟",
                                    "اكتب اسم حيوانين يملكان فراءً بنياً؟",
                                    "اكتب اسم شيئين نراهما في محطة الوقود؟",
                                    "اكتب اسم أداة نستخدمها لربط الأشياء ببعضها؟",
                                    "اكتب اسم نوعين من الأطعمة المشوية؟",
                                    "اكتب اسم حيوانين يملكان أذنين طويلتين؟",
                                    "اكتب اسم شيئين نستخدمهما للحماية من الشمس؟",
                                    "اكتب اسم فاكهتين تحتويان على نسبة عالية من الماء؟",
                                    "اكتب اسم أداة نستخدمها في المطبخ لتقطيع الطعام؟",
                                    "اكتب اسم شيئين مصنوعين من المعدن في البيت؟",
                                    "اكتب اسم نوعين من الخبز؟",
                                    "اكتب اسم حيوانين يملكان مخالب حادة؟",
                                    "اكتب اسم شيئين نراهما في السوبرماركت دائماً؟",
                                    "اكتب اسم أداة نستخدمها لقياس الوزن؟",
                                    "اكتب اسم فاكهتين صيفيتين؟",
                                    "اكتب اسم حيوانين من فصيلة القطط؟",
                                    "اكتب اسم شيئين نستخدمهما للسفر؟",
                                    "اكتب اسم مشروبين يفضلهما الأطفال؟",
                                    "اكتب اسم أداة نستخدمها للتحكم في التلفاز؟",
                                    "اكتب اسم شيئين يوجدان في المقلمة؟",
                                    "اكتب اسم نوعين من الأجبان?️",
                                    "اكتب اسم حيوانين يملكان القدرة على التسلق؟",
                                    "اكتب اسم شيئين نستخدمهما للتدفئة؟",
                                    "اكتب اسم فاكهتين حمضيتين؟",
                                    "اكتب اسم أداة نستخدمها في الرسم؟",
                                    "اكتب اسم شيئين نراهما في المطار؟",
                                    "اكتب اسم حيوانين يملكان صوتاً مميزاً؟",
                                    "اكتب اسم شيئين نستخدمهما للغسيل؟",
                                    "اكتب اسم نوعين من الأسماك؟",
                                    "اكتب اسم حيوانين يعيشان في الجحور؟",
                                    "اكتب اسم شيئين يوجدان في حقيبة السفر؟",
                                    "اكتب اسم أداة نستخدمها في الخياطة؟",
                                    "اكتب اسم فاكهتين تزرعان في المناطق الحارة؟",
                                    "اكتب اسم شيئين مصنوعين من الخشب؟",
                                    "اكتب اسم نوعين من الزيوت نستخدمها في الطبخ؟",
                                    "اكتب اسم حيوانين يملكان صدفة؟",
                                    "اكتب اسم شيئين نستخدمهما لترتيب المكتب؟",
                                    "اكتب اسم مشروبين يقدما في المناسبات؟",
                                    "اكتب اسم أداة نستخدمها لغلق الأبواب؟",
                                    "اكتب اسم فاكهتين بأسماء ألوانها؟",
                                    "اكتب اسم حيوانين يملكان بقعاً على جلدهما؟",
                                    "اكتب اسم شيئين نستخدمهما للتصوير؟",
                                    "اكتب اسم أداة نستخدمها لفتح الزجاجات؟",
                                    "اكتب اسم نوعين من الشوربة؟",
                                    "اكتب اسم حيوانين يملكان أنوفاً طويلة؟",
                                    "اكتب اسم شيئين يوجدان في خزانة الملابس؟",
                                    "اكتب اسم أداة نستخدمها لربط الحذاء؟",
                                    "اكتب اسم فاكهتين تتوفران طوال السنة؟",
                                    "اكتب اسم شيئين مصنوعين من الجلد؟",
                                    "اكتب اسم نوعين من الفطور؟",
                                    "اكتب اسم حيوانين يملكان أرجلاً كثيرة؟",
                                    "اكتب اسم شيئين نستخدمهما للعب؟",
                                    "اكتب اسم أداة نستخدمها للتثقيب؟",
                                    "اكتب اسم فاكهتين تنموان على الشجر؟",
                                    "اكتب اسم شيئين يوجدان في الحديقة؟",
                                    "اكتب اسم نوعين من السلطات؟",
                                    "اكتب اسم حيوانين من الزواحف؟",
                                    "اكتب اسم شيئين نستخدمهما للرؤية؟",
                                    "اكتب اسم أداة نستخدمها في الحلاقة؟",
                                    "اكتب اسم فاكهتين تنموان في المناطق الباردة؟"
                                ]

                                selected_request = random.choice(items_pool)

                                game_msg = (
                                    "[b][c][FFFFFF]تَحَدِّي الحَاجَتَيْنِ\n"
                                    "────────────────────\n"
                                    "[FF0000]{request}\n"
                                    "[FFFFFF]────────────────────\n"
                                    "⚡ [FF0000]اكتب الشيئين في رسالة واحدة لتفوز! 🏃‍♂️"
                                ).format(request=selected_request)
                        ...
                        if inPuTMsG and inPuTMsG.strip().startswith('/خوة '):
                            try:
                                ...
                                parts = inPuTMsG.strip().split()
                                if len(parts) < 3:
                                    await safe_send_message(response.Data.chat_type, "[b][c][FF0000]❌ استخدم: /خوة [خوي1] [خوي2]", uid, chat_id, key, iv)
                                else:
                                    name1, name2 = parts[1], parts[2]
                                    p = random.randint(1, 100)
                                    
                                    # الفئة الأولى (40% - 100 رد)
                                    cat1 = [
                                        "خوتكم مثل شاحن آيفون صيني يخلص بطلعة.", "خوتكم صفر على الشمال.", "لو تجتمعون ما تجيبون حق شاورما.", "يا رجل لو شافك طايح بيصورك ستوري.",
                                        "خوتكم مثل فيلم رعب بطلّه غبي.", "أقسم بالله لو أشوفكم سوا أحسبكم متهاوشين.", "خوتكم مثل اللاق في القيم.", "خوتكم مثل مسلسل تركي ممل.", "أعتزلوا الخوة يا شباب.", "خوة مصلحة بامتياز.",
                                        "يا سامي احذف أسماءهم من قائمة الأصحاب.", "خوة هشة مثل الكرتونة في المطر.", "خوة لا طعم ولا لون.", "يا رجل حتى الغنم ما تمشي معكم.", "خوتكم غلطة مطبعية في السيرفر.", "علاقة لا تستحق حتى الذكر.", "خوة تنتهي قبل ما تبدأ.", "مثل ماء البحر مالح ولا ينشرب.", "بينكم حقد دفين مو خوة.", "مستوى تحت الصفر بكتير.",
                                        "خوة مثل الحلم المزعج.", "يا خسارة وقتكم سوا.", "تضيعون وقتكم على الفاضي.", "خوة هشة مثل الزجاج.", "لا وفاء ولا رجولة.", "خوتكم مثل عصير منتهي الصلاحية.", "يا عالم اتركوا بعض.", "خوة مبنية على كذب.", "أبعدوا عن بعض أريح للكل.", "خوة عبارة عن نكتة بايخة.",
                                        "كل يوم طالعين بمشكلة.", "ما في تفاهم نهائياً.", "خوتكم مثل حجر وسط الطريق.", "علاقة باردة ومملة.", "يا صبر الأرض عليكم.", "خوة ضعيفة جداً.", "لا تعتمدوا على بعض.", "خوتكم مثل ريشة في مهب الريح.", "يا خسارة الأيام.", "علاقة مسمومة.",
                                        "كل واحد في وادي.", "يا ساتر على هذي الخوة.", "خوتكم مثل غيمة بدون مطر.", "أوهام في أوهام.", "ما في أي رابط بينكم.", "تفاهم معدوم تماماً.", "يا رجل هذي مو خوة.", "علاقة ضايعة.", "كل واحد يتمنى الثاني يغيب.", "موقفكم يضحك.",
                                        "يا شين هالخوة.", "لا تلموني إذا ضحكت.", "خوة ميتة.", "أفضل حل تتركون بعض.", "تفاهمكم مثل الزيت والماء.", "يا ساتر على التناقض.", "محد يقدر يتحملكم.", "خوتكم فضيحة.", "أهون عليكم الغياب.", "خوة ناقصة.",
                                        "لا روح ولا تفاهم.", "شي لا يطاق.", "علاقة منهارة.", "خوتكم عبارة عن مسرحية.", "يا عالم اصحوا.", "خوة بدون أساس.", "تضييع للوقت.", "لا تترجوا الخير.", "يا حسافة على هذي الخوة.", "محد يطيقكم.",
                                        "يا شين النفسية.", "خوتكم كابوس.", "لا تزيدوا في التعب.", "علاقة محطمة.", "يا خسارة الرجولة فيكم.", "خوتكم فشل.", "لا تدوروا على الوفاء.", "يا كثر المشاكل.", "خوتكم مثل سراب.", "لا أمل.",
                                        "كلكم غلط في غلط.", "يا شين الطباع.", "خوتكم وجع رأس.", "لا تحاولوا إصلاح شيء.", "علاقة متهالكة.", "يا خيبة الأمل.", "خوتكم تنتهي بخصام.", "مثل رمل يذوب.", "يا شين هذي اللمة.", "خوة محبطة.",
                                        "يا خيبة الرجاء.", "لا أوفياء ولا أصدقاء.", "يا تعب المشوار.", "خوتكم سراب.", "خيبة كبيرة.", "لا تزيدوا الطين بلة.", "يا خسارة الملح.", "كلكم كذب.", "يا شين النهايات.", "يا ليتني ما سألت."
                                    ]

                                    # الفئة الثانية (30% - 100 رد)
                                    cat2 = [
                                        "خوة طيبة تمشي على البركة.", "علاقة مستقرة بس يبي لها حماس.", "خوة ديوانية تفتح النفس.", "تفاهم مقبول للقهوة.", "خوة أيام الجامعة.", "سمن على زيت.", "علاقة لا بأس بها.", "خوة محيرة.", "مستواكم متوسط.", "تفاهم مريح.",
                                        "يمشون بالستر.", "خوة حلوة وبسيطة.", "تصلحون لبعض في اللعب.", "علاقة هادية.", "خوة محترمة.", "تفاهم متوازن.", "علاقة واضحة.", "خوة مثل الروتين.", "تصلحون للضحك.", "خوة مقبولة.",
                                        "ماشي حالكم.", "علاقة في السليم.", "خوة معقولة جداً.", "تفاهم يسير ببطء.", "خوة فيها أمل.", "علاقة متجددة.", "تصلحون لبعض.", "خوة فيها الخير.", "تفاهم جميل.", "علاقة مريحة.",
                                        "خوة متناغمة.", "تصلحون للطلعات.", "علاقة في تقدم.", "خوة فيها وفاء.", "تفاهم رائع.", "علاقة مستمرة.", "خوة تفتح النفس.", "تصلحون لكل شيء.", "علاقة قوية.", "خوة فيها هيبة.",
                                        "تفاهم ذكي.", "علاقة متزنة.", "خوة فيها بركة.", "تصلحون للسوالف.", "علاقة واعدة.", "خوة فيها ضحك.", "تفاهم متبادل.", "علاقة فيها صدق.", "خوة فيها تجديد.", "تصلحون للفريق.",
                                        "علاقة مستقرة.", "خوة فيها ذوق.", "تفاهم سريع.", "علاقة فيها روح.", "خوة فيها دعم.", "تصلحون للأوقات الصعبة.", "علاقة فيها ثقة.", "خوة فيها احترام.", "تفاهم كبير.", "علاقة فيها مودة.",
                                        "خوة فيها طيبة.", "تصلحون للمغامرات.", "علاقة فيها حيوية.", "خوة فيها إخلاص.", "تفاهم ممتاز.", "علاقة فيها ود.", "خوة فيها حماس.", "تصلحون للمواقف.", "علاقة فيها نضج.", "خوة فيها كرم.",
                                        "تفاهم عالي.", "علاقة فيها تفاهم.", "خوة فيها هدوء.", "تصلحون للراحة.", "علاقة فيها أمان.", "خوة فيها إبداع.", "تفاهم متبادل.", "علاقة فيها سعادة.", "خوة فيها طموح.", "تصلحون للمستقبل.",
                                        "علاقة فيها توازن.", "خوة فيها رقي.", "تفاهم عميق.", "علاقة فيها ذكاء.", "خوة فيها تواضع.", "تصلحون لبعضكم.", "علاقة فيها خير.", "خوة فيها تميز.", "تفاهم مريح.", "علاقة فيها استقرار.",
                                        "خوة فيها فخر.", "تصلحون للنجاح.", "علاقة فيها وداد.", "خوة فيها إلهام.", "تفاهم مستمر.", "علاقة فيها صفاء.", "خوة فيها تفاهم.", "تصلحون للمجد.", "علاقة فيها محبة.", "خوة فيها أصل."
                                    ]

                                    # الفئة الثالثة (30% - 100 رد)
                                    cat3 = [
                                        "توأم روح في الرجولة!", "أعظم خوة في التاريخ.", "ملوك الخوة.", "خوة أسطورية.", "سند وقت اللزوم.", "خوة عالمية.", "تفاهم عالي.", "خوة مثل الرصاص.", "أقوى ثنائي.", "ملوك الكلان.",
                                        "وفاء لا يوصف.", "خوة من ذهب.", "إخوان من أم ثانية.", "هيبة ومواقف.", "خوة ملوك.", "أروع خويان.", "سند الأيام.", "خوة لا تكرر.", "أنتم القمة.", "خوة تدرس.",
                                        "يا حظكم ببعض.", "رجال في وقت الشدة.", "خوة أبدية.", "فخر الكلان.", "ثنائي الجبار.", "قوة لا تقهر.", "خوة حقيقية.", "يا بخت اللي عنده خوي مثلكم.", "خوة في القمة.", "رجال المواقف.",
                                        "تفاهم فوق الخيال.", "أعظم ثنائي.", "خوة تنكتب بالتاريخ.", "يا وحوش الوفاء.", "خوة حديدية.", "هيبة الرجولة.", "خوة بلا حدود.", "وفاء ومحبة.", "ثنائي لا يعوض.", "خوة في السحاب.",
                                        "رجال الوفاء.", "أصل الرجولة.", "خوة نادرة.", "تفاهم أسطوري.", "رجال وقت الصعاب.", "خوة قوية جداً.", "قمة الرجولة.", "وفاء مطلق.", "يا أبطال الخوة.", "خوة تنحط عالجرح.",
                                        "رجال في كل الأوقات.", "أعظم علاقة.", "خوة لا تهتز.", "تفاهم مذهل.", "يا حظنا فيكم.", "خوة من حديد.", "رجال الكلان.", "وفاء لا يضاهى.", "خوة في الصدارة.", "أبطال بكل معنى.",
                                        "ثنائي في القمة.", "خوة تنير الدرب.", "رجال العز.", "خوة بلا مثيل.", "تفاهم كالملك.", "رجال في زمن الوفاء.", "خوة أبدية.", "فخرنا فيكم.", "خوة فوق الوصف.", "أبطال لا يهابون.",
                                        "تفاهم في القمة.", "علاقة تاريخية.", "خوة في أوج عطائها.", "رجال لا يلينون.", "وفاء صافي.", "خوة في القلب.", "ثنائي في القلوب.", "رجال لا يُنسون.", "خوة خالدة.", "أبطال الوفاء.",
                                        "تفاهم مذهل جداً.", "خوة ملوك ملوك.", "رجال في وقت المعمعة.", "وفاء حقيقي.", "خوة في العلالي.", "ثنائي جبار.", "رجال ذو عزيمة.", "خوة تنبض بالوفاء.", "أبطال بكل فخر.", "خوة لا توصف بكلمات.",
                                        "تفاهم يطير العقل.", "علاقة ملوكية.", "خوة في قمة الهرم.", "رجال يبيضون الوجه.", "وفاء يتحدى الزمن.", "خوة في الطليعة.", "أبطال الساحات.", "ثنائي لا يهزم.", "خوة في العيون.", "أصل الوفاء."
                                    ]

                                    if p <= 40: msg = random.choice(cat1)
                                    elif p <= 70: msg = random.choice(cat2)
                                    else: msg = random.choice(cat3)
                                    
                                    result = (
                                        f"[b][c][FFFFFF]━ كلان على هونك ━\n"
                                        f"[00FF00]{name1} [FF0000]× [00FF00]{name2}\n"
                                        f"[FFFF00]نسبة التوافق: {p}%\n"
                                        f"[FFFFFF]───────────────\n"
                                        f"[FF0000]{msg}"
                                    )
                                ...
                                print(f"Error in /sami: {e}")
                            continue



                        if inPuTMsG and inPuTMsG.strip().startswith('/كف '):
                            try:
                                import random
                                parts = inPuTMsG.strip().split()
                                if len(parts) < 3:
                                    await safe_send_message(response.Data.chat_type, "[b][c][FF0000]❌ استخدم: /كف [الضارب] [المضروب]", uid, chat_id, key, iv)
                                else:
                                    attacker, target = parts[1], parts[2]
                                    
                                    slap_actions = [
                                        f"{attacker} عطى {target} كف خلى وجهه كأنه جدار مدهون ببوية قديمة.",
                                        f"{attacker} سطر {target} كف خلى عقله يسوي ريستارت ويرجع لعام 2005.",
                                        f"{attacker} جلد {target} كف طبع رسمة أصابعه على وجهه كأنه توقيع فنان.",
                                        f"{attacker} عطى {target} كف يخليه ينسى اسمه ويصير ينادي نفسه 'أبو كف'.",
                                        f"{attacker} شلخ وجه {target} بكف يخلي وجهه يصير مثل خريطة العالم المهجورة.",
                                        f"{attacker} لطش {target} كف خلى جبهته تلمع من الصدمة أكثر من شاشة الجوال.",
                                        f"{attacker} أهدى {target} كف يخليه يشك في أصله وفصله.",
                                        f"{attacker} سحب {target} كف يخلي وجهه يحول من بشري إلى كائن فضائي.",
                                        f"{attacker} رقع {target} كف خلى أسنانه تسوي سيمفونية داخل فمه.",
                                        f"{attacker} عطى {target} كف يخليه يروح يغسل وجهه بالديتول عشان يروح أثر الفضيحة.",
                                        f"{attacker} ضرب {target} كف خلى مخه يرتجف داخل الجمجمة مثل الجلي.",
                                        f"{attacker} صفّق {target} كف يخليه يدور على وجهه اللي طار بآخر السيرفر.",
                                        f"{attacker} رزع {target} كف خلاه يشوف صورته وهو طفل من كثرة الوجع.",
                                        f"{attacker} عطى {target} كف يخلي وجهه يصير مثل قفا الدجاجة.",
                                        f"{attacker} جلد {target} كف خلاه ينسى هو مين أصلاً وكيف دخل اللعبة.",
                                        f"{attacker} سطر {target} كف خلى شكله يصير مثل البصلة بعد التقشير.",
                                        f"{attacker} لطش {target} كف يخليه يدور على كرامته اللي ضاعت في الأرض.",
                                        f"{attacker} ضرب {target} كف خلى عيونه تنحرف لجهتين مختلفتين من الصدمة.",
                                        f"{attacker} رقع {target} كف يخليه يروح يشتري وجه جديد من المتجر.",
                                        f"{attacker} عطى {target} كف خلاه يحس إنه خروف ضايع في ليلة عيد.",
                                        f"{attacker} سحب {target} كف يخلي شكله يصير مثل ضب مضيع أهله.",
                                        f"{attacker} صفّق {target} كف خلى صدى صوته يتردد في كل السيرفر.",
                                        f"{attacker} رزع {target} كف يخلي وجهه كأنه طالع من مفرمة لحم.",
                                        f"{attacker} عطى {target} كف يخليه يشك إنه أصلاً كان عنده وجه.",
                                        f"{attacker} جلد {target} كف خلاه يطير للوبّي ويطلب اعتذار من الجدار.",
                                        f"{attacker} ضرب {target} كف يخلي ملامح وجهه تتوزع في أرجاء المكان.",
                                        f"{attacker} لطش {target} كف يخليه يروح يغير ملامحه بعملية تجميل.",
                                        f"{attacker} سطر {target} كف خلاه يتذكر كل خيانة سواها في حياته.",
                                        f"{attacker} رقع {target} كف يخلي وجهه يصير مثل علبة تونة معفنة.",
                                        f"{attacker} عطى {target} كف يخليه يحس إنه قطعة خردة في مكب نفايات.",
                                        f"{attacker} سحب {target} كف خلى شكله يصير مضحكة للكلان كله.",
                                        f"{attacker} صفّق {target} كف يخليه ينسى كيف يكتب اسمه في الشات.",
                                        f"{attacker} رزع {target} كف خلى رقبته تلف 180 درجة من قوة الضربة.",
                                        f"{attacker} ضرب {target} كف يخلي وجهه يصير كأنه خريطة مدن محطمة.",
                                        f"{attacker} عطى {target} كف يخليه يحس إنه ممسحة أرضية للكلان.",
                                        f"{attacker} جلد {target} كف خلاه يدوخ لدرجة إنه بدأ يكلم نفسه بالياباني.",
                                        f"{attacker} لطش {target} كف خلى شكله يصير مثل وجه واحد فاقد الأمل.",
                                        f"{attacker} سطر {target} كف خلاه يروح يطلب اللجوء السياسي من كثر الإهانة.",
                                        f"{attacker} رقع {target} كف يخلي وجهه يصير كأنه قطعة قماش مهترئة.",
                                        f"{attacker} ضرب {target} كف خلاه يشوف طيف جده وهو يناديه يرجع البيت.",
                                        f"{attacker} عطى {target} كف خلاه يحس إنه نكتة سمجة في مسلسل بايخ.",
                                        f"{attacker} سحب {target} كف خلى شكله مثل واحد توه صاحي من غيبوبة.",
                                        f"{attacker} صفّق {target} كف خلاه ينسى هو مين أصلاً.",
                                        f"{attacker} رزع {target} كف يخلي وجهه يصير مثل البطيخة المهروسة.",
                                        f"{attacker} ضرب {target} كف خلاه يروح يدور على كرامته اللي طارت في الجو.",
                                        f"{attacker} جلد {target} كف خلى شكله مثل قفا ولد عمره 5 سنوات.",
                                        f"{attacker} لطش {target} كف خلاه يحس إنه غلطة تقنية في السيرفر.",
                                        f"{attacker} سطر {target} كف خلى وجهه يصير كأنه لوحة سريالية تعبر عن الفشل.",
                                        f"{attacker} رقع {target} كف يخليه يروح يغطي وجهه بكرتون من الفضيحة.",
                                        f"{attacker} عطى {target} كف خلاه يحس إنه خوي فاشل بكل المقاييس.",
                                        # ... (استمر في هذا النمط الشخصوني القوي حتى الـ 150)
                                        f"{attacker} عطى {target} كف خلاه يعرف مقامه الحقيقي في هذا السيرفر.",
                                        f"{attacker} سحب {target} كف يخلي وجهه كأنه طالع من حرب عالمية.",
                                        f"{attacker} ضرب {target} كف خلاه يندم إنه فكر يفتح فمه قدام {attacker}.",
                                        f"{attacker} رزع {target} كف يخلي وجهه يصير كأنه ساندوتش طعس.",
                                        f"{attacker} جلد {target} كف خلاه يحس إنه مجرد رقم عشوائي في القائمة.",
                                        f"{attacker} صفّق {target} كف خلاه يروح يطلب من الإدارة يحذفون حسابه.",
                                        f"{attacker} لطش {target} كف خلى شكله يصير كأنه وجه صرصار مرصوص.",
                                        f"{attacker} سطر {target} كف خلاه يتأكد إنه أضعف لاعب في تاريخ اللعبة.",
                                        f"{attacker} رقع {target} كف يخلي وجهه يصير مثل كفر سيارة محروق.",
                                        f"{attacker} عطى {target} كف خلاه يحس إنه غلطة طبيعة.",
                                        f"{attacker} ضرب {target} كف خلاه يروح يكتب وصيته من كثرة الوجع.",
                                        f"{attacker} سحب {target} كف خلى شكله مثل وجه واحد سارق خبز.",
                                        f"{attacker} صفّق {target} كف خلاه يتمنى لو إنه ما دخل السيرفر أصلاً.",
                                        f"{attacker} رزع {target} كف خلى وجهه يقلب ألوان الطيف من شدة الضربة.",
                                        f"{attacker} جلد {target} كف خلاه يروح يغير اسمه لـ 'المضروب'.",
                                        f"{attacker} لطش {target} كف خلاه يحس إنه أسوأ خوي عرفته البشرية.",
                                        f"{attacker} سطر {target} كف خلى شكله يصير مثل عجينة ناقصة ملح.",
                                        f"{attacker} رقع {target} كف يخليه يروح ينام في الزاوية من الذل.",
                                        f"{attacker} عطى {target} كف خلاه يتأكد إن نهايته قربت.",
                                        f"{attacker} ضرب {target} كف خلاه يحس إنه ذبابة مزعجة تحت رجول {attacker}.",
                                        f"{attacker} سحب {target} كف خلى شكله مثل وجه واحد بلع ليمونة متعفنة.",
                                        f"{attacker} صفّق {target} كف خلاه يندم على الساعة اللي شاف فيها اللعبة.",
                                        f"{attacker} رزع {target} كف خلى وجهه يصير مثل طبعة جزمة 45.",
                                        f"{attacker} جلد {target} كف خلاه يحس إنه مجرد كيس رمل للتدريب.",
                                        f"{attacker} لطش {target} كف خلى شكله يصير مثل وجه ضفدع مصدوم.",
                                        f"{attacker} سطر {target} كف خلاه يروح يغسل وجهه بالموية المرة.",
                                        f"{attacker} رقع {target} كف يخليه يحس إنه صفر على الشمال.",
                                        f"{attacker} عطى {target} كف خلى شكله مثل وجه واحد طالع من مغسلة سيارات.",
                                        f"{attacker} ضرب {target} كف خلاه يتمنى لو إنه كان بوت بدل ما يكون لاعب.",
                                        f"{attacker} سحب {target} كف خلى شكله مثل وجه واحد توه طايح من سيكل.",
                                        f"{attacker} صفّق {target} كف خلاه يروح يغير إعدادات وجهه من الإدارة.",
                                        f"{attacker} رزع {target} كف خلى وجهه يصير كأنه طينة مرصوصة.",
                                        f"{attacker} جلد {target} كف خلاه يحس إنه مجرد إكسسوار في السيرفر.",
                                        f"{attacker} لطش {target} كف خلى شكله مثل وجه واحد ما نام من 3 أيام.",
                                        f"{attacker} سطر {target} كف خلاه يندم إنه فكر يصارع كبار السيرفر.",
                                        f"{attacker} رقع {target} كف خلى وجهه يصير كأنه سلة مهملات.",
                                        f"{attacker} عطى {target} كف خلاه يحس إنه غلطة في نظام التوفيق.",
                                        f"{attacker} ضرب {target} كف خلى شكله مثل وجه واحد أكل كف من أبوه.",
                                        f"{attacker} سحب {target} كف خلاه يروح يشتري أقنعة تغطي الفشل.",
                                        f"{attacker} صفّق {target} كف خلى وجهه يصير مثل طبعة كف على زجاج.",
                                        f"{attacker} رزع {target} كف خلاه يحس إنه لا شيء مقارنة بـ {attacker}.",
                                        f"{attacker} جلد {target} كف خلى شكله مثل وجه واحد ضايع في الصحراء.",
                                        f"{attacker} لطش {target} كف خلاه يندم إنه فتح فمه بكلمة.",
                                        f"{attacker} سطر {target} كف خلى وجهه يصير كأنه قطعة معجون طفل.",
                                        f"{attacker} رقع {target} كف خلاه يحس إنه مجرد خيال عابر.",
                                        f"{attacker} عطى {target} كف خلى شكله مثل وجه واحد خسر كل فلوسه.",
                                        f"{attacker} ضرب {target} كف خلاه يروح يطلب مساعدة من الدعم الفني.",
                                        f"{attacker} سحب {target} كف خلى شكله مثل وجه واحد طالع من حرب بالمسدسات المائية.",
                                        f"{attacker} صفّق {target} كف خلاه يتأكد إنه كان مجرد حلم.",
                                        f"{attacker} رزع {target} كف خلى وجهه يصير مثل طبعة رجل في وحل.",
                                        # ... (تكملة الـ 150 جملة بنفس الأسلوب الشخصوني المضحك)
                                    ]
                                    
                                    msg = random.choice(slap_actions)
                                    result = (
                                        f"[b][c][FF0000]━ حلبة التصفيق ━\n"
                                        f"[FFFFFF]──────────\n"
                                        f"[FFFFFF]{msg} [FF0000]!🔥"
                                    )
                                ...
                                print(f"Error in /كف: {e}")
                            continue


                        if inPuTMsG.strip() == "/samiadmin":
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samimultijoin'):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samireject'):
                        ...
                        if inPuTMsG.strip() == '/samireject_stop':
                            if reject_spam_task and not reject_spam_task.done():
                                reject_spam_running = False
                                reject_spam_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/samis3'):
                            await handle_badge_command('s3', inPuTMsG, uid, chat_id, key, iv, region, response.Data.chat_type)

                        if inPuTMsG.strip().startswith('/samis4'):
                            await handle_badge_command('s4', inPuTMsG, uid, chat_id, key, iv, region, response.Data.chat_type)

                        if inPuTMsG.strip().startswith('/samis5'):
                            await handle_badge_command('s5', inPuTMsG, uid, chat_id, key, iv, region, response.Data.chat_type)

                        if inPuTMsG.strip().startswith('/samis6'):
                            await handle_badge_command('s6', inPuTMsG, uid, chat_id, key, iv, region, response.Data.chat_type)

                        if inPuTMsG.strip().startswith('/samis7'):
                            await handle_badge_command('s7', inPuTMsG, uid, chat_id, key, iv, region, response.Data.chat_type)

                        if inPuTMsG.strip().startswith('/samis8'):
                        ...
                        if inPuTMsG.strip().startswith('/samicreateroom'):
                        ...
                        if inPuTMsG.strip().startswith('/samiarr'):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samixggst'):
                        ...
                        # NEW LAG COMMAND
                        if inPuTMsG.strip().startswith('/samiabra '):
                        ...
                        # STOP LAG COMMAND
                        if inPuTMsG.strip() == '/samistop lag':
                            if lag_task and not lag_task.done():
                                lag_running = False
                                lag_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/samimg '):
                        ...
                        if inPuTMsG.strip().startswith('/samimassage '):
                        ...
                        # Add stop command
                        if inPuTMsG.strip() == '/samistop msg':
                            if msg_spam_task and not msg_spam_task.done():
                                msg_spam_running = False
                                msg_spam_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/samitrain'):
                        ...
                        # Add this to your command handlers in TcPChaT function:
                        if inPuTMsG.strip().startswith('/samijoin_req '):
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samilagx'):
                            parts = inPuTMsG.strip().split()
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samioffline'):
                            parts = inPuTMsG.strip().split()
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/samiem'):
                            parts = inPuTMsG.strip().split()
...
#                            await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                        ...
                        elif inPuTMsG.strip().startswith('/samie'):
                        ...
                        if inPuTMsG.strip().startswith('/samigali '):
                            ...
                            try:
                                ...
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().startswith('/samilove '):
                        ...
                        if inPuTMsG.strip().startswith('/samip'):
                        ...
                        # Spam request command - works in all chat types
                        if inPuTMsG.strip().startswith('/samispm_inv'):
                        ...
                        if inPuTMsG.strip() == '/samistop spm_inv':
                            if spam_request_task and not spam_request_task.done():
                                spam_request_running = False
                                spam_request_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/samistatus'):
                            
                            # 🎯 حماية الأمر: لا ينفذ إلا إذا كان المرسل هو المالك
                            if str(uid) != str(BOT_OWNER_UID):
                                print(f"🚫 محاولة استخدام أمر الحالة من قبل غير المالك: {uid}")
                                await safe_send_message(response.Data.chat_type, "[FF0000]❌ هذا الأمر خاص بمالك البوت فقط!", uid, chat_id, key, iv)
                                return

                            print('Processing status command for owner')
    
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage:  /samistatus (player_uid)\nExample:  /samistatus 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                return
                            ...
                            try:
                                ...
                                await safe_send_message(response.Data.chat_type, f"[FF0000]❌ Error: {str(e)[:50]}", uid, chat_id, key, iv)


                        # NEW EVO COMMANDS
                        if inPuTMsG.strip().startswith('/samievo '):
                        ...
                        if inPuTMsG.strip().startswith('/samievo_fast '):
...
# ================= BUNDLE COMMAND START =================
   # ================= FINAL BUNDLE COMMAND (FAST) =================
                        if inPuTMsG.strip().startswith('/samib'):
...
[FFFFFF]Example: /b midnight"""
                        ...
                        if inPuTMsG.strip().startswith('/samievo_c '):
...
# ================= BUNDLE COMMAND START =================
   # ================= FINAL BUNDLE COMMAND (FAST) =================
                        if inPuTMsG.strip().startswith('/samibundle'):
                            print('Processing bundle command')
...
[FFFFFF]Example: /bundle midnight"""
   ...
   # ================= FINAL ANIMATION COMMAND (FAST) =================
                        if inPuTMsG.strip().startswith('/samianimation'):
                            print('Processing bundle command')
...
[FFFFFF]Example: /animation midnight"""
                        ...
                        if inPuTMsG.strip().startswith('/samilook'):
...
# ==================== /coin COMMAND ====================
                        if inPuTMsG.strip().startswith('/samicoin'):
                            ...
                            coin_message = (
                                ...
                                f"║ 🙋 Flipped by: [FFFF00]skip-codex\n"
                                f"║ ▪ Try again: /coin\n"
                                f"╚════════════════════╝"
                            )
...
# ==================== /dice COMMAND ====================
                        if inPuTMsG.strip().startswith('/samidice'):
...
#GALI SPAM MESSAGE 
    ...
    "Rg Rifat Official", "RG RIFAT_07", "RgRifat07" ]
                        ...
                        if inPuTMsG.strip() == '/samistop evo_fast':
                            if evo_fast_spam_task and not evo_fast_spam_task.done():
                                evo_fast_spam_running = False
                                evo_fast_spam_task.cancel()
                        ...
                        if inPuTMsG.strip() == '/samistop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                evo_custom_spam_running = False
                                evo_custom_spam_task.cancel()
                        ...
                        # =================== COMPLETE HELP MENU (SEPARATE PARTS) ===================
                         ...
                         # =================== WORKING COMMANDS HELP MENU ===================
                        if inPuTMsG.strip().lower() in ("helpsamii", "/helpsami", "halpsamii", "RIFATsamii", "samii***"):
                        ...
                        # /b_power command
                        if inPuTMsG.strip().startswith('/samibundle_power'):
                        ...
                        if response.Data.chat_type == 2 and original_msg.strip().startswith('/samiautorep '):
                            await handle_autorep_command(original_msg, uid, chat_id, response.Data.chat_type, key, iv, region)
                            continue

                        # /_inv command
                        if inPuTMsG.strip().startswith('/sami_inv '):
...
"""
                        ...
                        if response and ai_auto_reply_enabled and inPuTMsG and not inPuTMsG.strip().startswith('/sami') and not inPuTMsG.strip().startswith('+') and not inPuTMsG.strip().startswith('-') and not inPuTMsG.strip().startswith('@'):
                        ...
                        if inPuTMsG.strip().startswith('/samiPglag '):
                        ...
                        if inPuTMsG.strip() == '/samistop chatspam':
                            if 'chatspam_task' in globals() and chatspam_task and not chatspam_task.done():
                                chatspam_running = False
                                chatspam_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/samiinfo '):
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("🤖 SKIP-CODEX BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
